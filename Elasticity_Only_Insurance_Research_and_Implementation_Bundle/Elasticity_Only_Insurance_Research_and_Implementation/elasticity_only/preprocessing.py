...
class FeatureSpecification:
    ...
    def validate(self, frame: pd.DataFrame) -> None:
        validate_feature_list(self.all)
        missing = [c for c in self.all if c not in frame.columns]
        if missing:
            raise ValueError(f'feature columns missing from data: {missing}')
        overlap = set(self.numeric).intersection(self.categorical)
        if overlap:
            raise ValueError(f'columns listed as numeric and categorical: {sorted(overlap)}')
...
def make_sklearn_preprocessor(features: FeatureSpecification, *, scale_numeric: bool, one_hot_min_frequency: int | float | None=10, sparse_output: bool=True) -> ColumnTransformer:
    ...
    if scale_numeric:
        numeric_steps.append(('scale', StandardScaler(with_mean=True)))
...
def prepare_native_categoricals(train: pd.DataFrame, other: Iterable[pd.DataFrame], features: FeatureSpecification) -> tuple[pd.DataFrame, list[pd.DataFrame]]:
    features.validate(train)
    train_out = train.loc[:, features.all].copy()
    others = [f.loc[:, features.all].copy() for f in other]
    for c in features.categorical:
        cats = pd.Index(train_out[c].dropna().astype(str).unique())
        train_out[c] = pd.Categorical(train_out[c].astype('string'), categories=cats)
        for f in others:
            f[c] = pd.Categorical(f[c].astype('string'), categories=cats)
