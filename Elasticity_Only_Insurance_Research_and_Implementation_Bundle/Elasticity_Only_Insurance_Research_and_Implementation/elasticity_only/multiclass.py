...
class IPWMulticlassClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, estimator: Any, enforce_monotonicity: bool=True) -> None:
    ...
    def fit(self, X: Any, y: ArrayLike, *, pi: ArrayLike=DEFAULT_PI, sample_weight: ArrayLike | None=None) -> 'IPWMulticlassClassifier':
        ...
        w = base / p[np.arange(len(t)), t]
        w /= np.mean(w)
        self.model_ = clone(self.estimator)
        _fit_with_sample_weight(self.model_, X, t, w)
    ...
    def predict_normalized_demand(self, X: Any) -> FloatArray:
        ...
        for j, c in enumerate(self.model_.classes_.astype(int)):
            ordered[:, c] = proba[:, j]
        ordered = np.clip(ordered, 1e-09, None)
        ordered /= ordered.sum(1, keepdims=True)
        return ordered

    def predict_log_rr(self, X: Any) -> tuple[FloatArray, FloatArray]:
        s = self.predict_normalized_demand(X)
        h = np.log(s[:, 0]) - np.log(s[:, 1])
        l = np.log(s[:, 2]) - np.log(s[:, 1])
