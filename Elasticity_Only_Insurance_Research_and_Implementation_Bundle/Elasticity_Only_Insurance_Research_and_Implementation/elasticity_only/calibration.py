...
class PairwiseAffineCalibrator:
    ...
    def __init__(self, direction: Literal['high', 'low', 'none']='none', smooth_constraint: bool=True, constraint_temperature: float=0.05, max_slope: float=4.0, intercept_bound: float=12.0) -> None:
        self.direction = direction
        self.smooth_constraint = smooth_constraint
        self.constraint_temperature = constraint_temperature
        self.max_slope = max_slope
        self.intercept_bound = intercept_bound
    ...
    def _constrain(self, g: FloatArray) -> FloatArray:
        ...
        if self.direction == 'high':
            return project_log_rr_sign(g, np.zeros_like(g), mode=mode, temperature=self.constraint_temperature)[0]
        if self.direction == 'low':
            return project_log_rr_sign(np.zeros_like(g), g, mode=mode, temperature=self.constraint_temperature)[1]
    ...
    def fit(self, raw_log_rr: ArrayLike, y: ArrayLike, log_propensity_odds: ArrayLike, sample_weight: ArrayLike | None=None) -> 'PairwiseAffineCalibrator':
        raw = np.asarray(raw_log_rr, float).reshape(-1)
        yy = np.asarray(y, float).reshape(-1)
        offset = np.asarray(log_propensity_odds, float).reshape(-1)
        if not len(raw) == len(yy) == len(offset) or np.any(~np.isin(yy, [0.0, 1.0])):
            raise ValueError('aligned binary calibration inputs required')
        w = np.ones(len(raw)) if sample_weight is None else np.asarray(sample_weight, float).reshape(-1)
        if len(w) != len(raw) or np.any(w < 0) or (not np.any(w > 0)):
            raise ValueError('invalid sample weights')
        self.center_ = float(np.average(raw, weights=w))
        centered = raw - self.center_

        def global_obj(a):
            return float(np.average(binary_log_loss_rows(yy, offset + self._constrain(np.full(len(raw), a[0]))), weights=w))
        gf = minimize(global_obj, np.array([0.0]), method='L-BFGS-B', bounds=[(-self.intercept_bound, self.intercept_bound)])

        def objective(theta):
            g = self._constrain(theta[0] + np.exp(theta[1]) * centered)
            return float(np.average(binary_log_loss_rows(yy, offset + g), weights=w))
        res = minimize(objective, np.array([float(gf.x[0]), np.log(0.5)]), method='L-BFGS-B', bounds=[(-self.intercept_bound, self.intercept_bound), (np.log(1e-05), np.log(self.max_slope))])
        self.intercept_ = float(res.x[0])
        self.slope_ = float(np.exp(res.x[1]))
    ...
    def predict_log_rr(self, raw_log_rr: ArrayLike, *, hard_final_constraint: bool=True) -> FloatArray:
        ...
        if hard_final_constraint and self.direction == 'high':
            return np.minimum(g, 0.0)
        if hard_final_constraint and self.direction == 'low':
