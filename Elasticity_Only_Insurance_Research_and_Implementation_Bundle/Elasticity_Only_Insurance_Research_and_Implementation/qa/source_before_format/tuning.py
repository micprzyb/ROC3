...
def create_study(
    ...
    *,
    ...
    pruner = optuna.pruners.MedianPruner(
        n_startup_trials=max(10, n_startup_trials // 2),
        n_warmup_steps=1,
        n_min_trials=5,
    )
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        for step, (train_index, valid_index) in enumerate(splits):
            ...
            fit_kwargs: dict[str, Any] = {
                "pi": pi[train_index],
                "eval_set": (row_subset(X, valid_index), arm[valid_index]),
                "eval_pi": pi[valid_index],
            }
            if sample_weight is not None:
                fit_kwargs["sample_weight"] = sample_weight[train_index]
                fit_kwargs["eval_sample_weight"] = sample_weight[valid_index]
            model.fit(row_subset(X, train_index), arm[train_index], **fit_kwargs)
            ...
            score = pairwise_nll(
                ...
                pred,
                contrast_arm,
                None if sample_weight is None else sample_weight[valid_index],
            )
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        for step, (train_index, valid_index) in enumerate(splits):
            ...
            score = float(
                np.average(
                    rows,
                    weights=None if sample_weight is None else sample_weight[valid_index],
                )
            )
            scores.append(score)
            trial.report(float(np.mean(scores)), step)
