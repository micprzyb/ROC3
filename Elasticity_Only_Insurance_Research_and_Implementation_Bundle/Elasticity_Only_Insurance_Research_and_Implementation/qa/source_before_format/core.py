...
DEFAULT_PI = np.array([0.10, 0.80, 0.10], dtype=np.float64)
DEFAULT_MULTIPLIERS = np.array([1.10, 1.00, 0.90], dtype=np.float64)
...
def validate_propensities(pi: ArrayLike, n: int | None = None) -> FloatArray:
    ...
    if not np.allclose(arr.sum(axis=1), 1.0, atol=1e-8, rtol=1e-8):
...
def stable_logit(p: ArrayLike, eps: float = 1e-9) -> FloatArray:
...
def risk_ratios_from_buyer_posterior(posterior: ArrayLike, pi: ArrayLike = DEFAULT_PI) -> FloatArray:
    ...
    if np.any(r <= 0) or not np.allclose(r.sum(axis=1), 1.0, atol=1e-8):
